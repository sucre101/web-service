...
export default {
  ...
  methods: {
    changeScreen(currentTabIndex) {
      ...
      if (this.$route.query.tab !== this.replaceRouteTitle() || Object.keys(this.$route.query).length > 1) {
        ...
        this.$router.replace(
            { query: { tab: this.replaceRouteTitle() }}
        )
      }
    ...
    replaceRouteTitle() {
      return this.componentList[this.currentIndex].title.toLocaleLowerCase().replace(' ', '_')
    }
